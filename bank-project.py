...
def returnToHome(master):
	master.destroy()
	main()

def write(master,name,oc,pin):
	
	if( (numericalExist(name)) or (numericalExist(oc)==0) or (numericalExist(pin)==0)or name==""):
		messagebox.showinfo("Error","Invalid Credentials\nPlease try again.")
		master.destroy()
		return 
...
def write_credit(master,amt,accnt,name):

	if(numericalExist(amt)==0):
		messagebox.showinfo("Error","Invalid Credentials\nPlease try again.")
		master.destroy()
		return 
...
def write_debit(master,amt,accnt,name):
...
def debit_write(accnt,name):
	...
	l1.pack(side="top")
	e1.pack(side="top")
	b=tk.Button(creditwn,text="Credit",font=("Times",16),relief="raised",command=lambda:write_credit(creditwn,e1.get(),accnt,name))
	b.pack(side="top")
	creditwn.bind("<Return>",lambda x:write_credit(creditwn,e1.get(),accnt,name))


def debit_amount(accnt,name):
	debitwn=tk.Tk()
	...
	l1.pack(side="top")
	e1.pack(side="top")
	b=tk.Button(debitwn,text="Debit",font=("Times",16),relief="raised",command=lambda:write_debit(debitwn,e1.get(),accnt,name))
	b.pack(side="top")
	debitwn.bind("<Return>",lambda x:write_debit(debitwn,e1.get(),accnt,name))




def display_the__balance(accnt):
	fdet=open(accnt+".txt",'r')
	fdet.readline()
	bal=fdet.readline()
	fdet.close()
...
def loggedinto_menu(accnt,name):
	rootwn=tk.Tk()
	...
	b2.image=myimg2
	b3=tk.Button(image=myimg3,command=lambda: debit_amount(accnt,name))
	b3.image=myimg3
	b4=tk.Button(image=myimg4,command=lambda: display_the__balance(accnt))
	b4.image=myimg4
	b5=tk.Button(image=myimg5,command=lambda: display_hist(accnt))
	b5.image=myimg5
...
def checklogged_in(master,name,acc_num,pin):
	if(accontCheck(acc_num)==0):
		master.destroy()
		main()
		return

	if( (numericalExist(name))  or (numericalExist(pin)==0) ):
		messagebox.showinfo("Error","Invalid Credentials\nPlease try again.")
		master.destroy()
		main()
	else:
		master.destroy()
		loggedinto_menu(acc_num,name)


def logged_in(master):
	master.destroy()
	loginwn=tk.Tk()
	...
	e3.pack(side="top")
	b=tk.Button(loginwn,text="Submit",command=lambda: checklogged_in(loginwn,e1.get().strip(),e2.get().strip(),e3.get().strip()))
	b.pack(side="top")
	b1=tk.Button(text="HOME",font=("Times",16),relief="raised",bg="blue4",fg="white",command=lambda: returnToHome(loginwn))
	b1.pack(side="top")
	loginwn.bind("<Return>",lambda x:checklogged_in(loginwn,e1.get().strip(),e2.get().strip(),e3.get().strip()))
...
def get_created():
	...
	crwn.title("get_created Account")
...
def main():
	...
	b1=tk.Button(image=imgc,command=get_created)
	b1.image=imgc
	b2=tk.Button(image=imglog,command=lambda: logged_in(rootwn))
	b2.image=imglog
